...
function Widget({ title, content }) {
  ...
  return (
    <motion.div
      ...
      id="widgets"
    >
      <h3 className="text-xl font-semibold mb-3 text-indigo-700 flex items-center gap-2">{title}</h3>
      ...
      {open ? (
        ...
        <p className="text-gray-600 italic select-none">Haz clic para ver más información</p>
      )}
    </motion.div>
  );
}
